...
namespace PathFinder
{
    class CoreClass
    {
        ...
        public List<Node> CopiedOpenListTemp;
        //Constructor
        ...
        public void IMBA( State[,] map)
        {
            ...
            SearchGridForm F = new SearchGridForm( height, width, startState.X, startState.Y, endState.X, endState.Y);
            ...
            while (true)
            {
                ...
                int counter = 0;
                int[,] borders = new int[height, width];
                ...
                //Console.WriteLine("in coreclass, border states is {0}", counter);


                 if (NumberCuts==1)
                    F.btn(NumberCuts,null,borders, map);
                //else
                  //  F.btn(NumberCuts, CopiedOpenListTemp,borders,graphI);

                CopiedOpenListTemp =F.CopiedOpenListT;
                coutTime  += F.countSearch;
                CoreClass.ExpandedNodes += F.nExp;
        ...
        public void CutDefine(ref Point leftP, ref Point rightP)
        {
            ...
            else
            {
                ...
                //leftP.X -=10+ temp_sum - temp_sum * (temp_sum - downSide) / (temp_sum);
                //leftP.Y -=10+ temp_sum - temp_sum * (temp_sum - leftSide) / (temp_sum); ;
                //rightP.X +=10+ temp_sum - temp_sum * (temp_sum - upSide) / (temp_sum); ;
                //rightP.Y +=10+ temp_sum - temp_sum * (temp_sum - rightSide) / (temp_sum); 
                leftP.X -= 50;
                leftP.Y -=50 ;
                rightP.X +=50 ;
                rightP.Y += 50 ;
        ...
        void WriteResults()
        {
            StreamWriter Writer = new StreamWriter("c:\\benchmark\\results\\"+"JPS2_"+Program.mapName+".txt", true);
